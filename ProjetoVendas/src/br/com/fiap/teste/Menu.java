...
public class Menu {
	...
	static String usuario="RM78514";
	static String senha="250294";
	...
	public static void main(String[] args) {
		...
		try {	 
			...
			con=Conexao.controlarInstancia().getConnection(usuario, senha);
			...
			if (op1=='C') {
				...
				if(opcli =='C') {
					...
					Dados.mensagem(ClienteBO.gravar(cli,e,t,con));
					...
					Dao = new ClienteDAO();
					ClienteBeans cli= new ClienteBeans();
					cli=Dao.consultar(Dados.texto("Email:"), con);
					...
					List<ClienteBeans> lista= Dao.listaCliente(con);
			...
			if (op1=='P') {
				...
				if(opprod =='C') {
					...
					System.out.println(ProdutoBO.novoProduto(pb,con));
					...
					System.out.println(ProdutoBO.atualizaProduto(pb,con));

				}else if (opprod =='L') {

					dao = new ProdutoDAO();
					List<ProdutoBeans> lista = dao.listarProduto(con);
			...
			if (op1=='V') {
				...
				if(opvend =='C') {
					...
					VendaBO.attValor(vb.getCd(), nf,con);
					dao=new NotaFiscalDAO();
					dao.attValor(vb.getCd(), nf, con);
					...
					ItemVendaBO.deletarVenda(cd,con);
					JOptionPane.showMessageDialog(null, VendaBO.deletar(cd,con));
					...
					List<RetornoVenda> lista=VendaBO.consultar(con);
			...
			if (op1=='R') {
				...
				do {
					...
					List<ItemVendaBeans> lista=ItemVendaBO.maiorItemVenda(con);
